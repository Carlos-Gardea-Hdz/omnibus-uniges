# AGENTS.md — omnibus-uniges

> Canonical agent rulebook for this repo (open standard: https://agents.md).
> Read this file plus **SPEC.md** before any task.
...
## Coding law (precedence)
1. **Global vault** — `~/.claude/AGENTS.md` + on-demand modules in `~/.claude/.agent-rules/*.md`. This is **the law**; read it first.
2. **SPEC.md** — single source of truth for this project's domain rules. Supersedes the legacy MASTER-BLUEPRINT-UNIGES.md when they conflict.
3. **This file + ARCHITECTURE.md** — project context and target architecture.

When this file or SPEC conflicts with the global vault, the **vault wins** on coding law; **SPEC wins** on domain rules.

## Domains
- Production: titulacion.carlosgardea.com
- Staging:    titulacion.carlosgardea.cloud
- VPS path:   /opt/omnibus/projects/uniges/
- Repo:       git@github.com:Carlos-Gardea-Hdz/omnibus-uniges.git
